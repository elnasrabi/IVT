...
//     console.log('connotesToRelease',JSON.parse(connotesToRelease.connote))
  ...
  for (i = 0; i < list.length; i++) {
    const payload = {
      ...
      try {
        const response = await axios.post('https://localhost:5050/api/rules/deleteHeldConnote', payload ).then(response => {
  ...
  function ReleaseConnote(connote){
    ...
    const res =  axios.post('https://localhost:5050/api/rules/deleteHeldConnote', connote).then(response => {
